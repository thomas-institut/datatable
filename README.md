...
This package defines abstract data access and manipulation of SQL-like tables (tables made out of rows with a unique
integer id as
its key), and provides in-memory and MySQL implementations of that abstraction. The purpose is to decouple basic data
functions
...
Invalid argument errors are handled by custom exceptions (see each method's documentation for details), and all other
problems normally result in a RunTimeException being thrown.
...
uses a different name and the implementation does not allow to set this in its constructor, it can be set with
`setIdColumnName` right after construction:
...
Note: *Setting the column name in a DataTable does not change anything in the underlying database. It only tells the
DataTable
what the actual database id name is.*
...
Any number of fields/columns and types can be used in the creation row as long as this makes
...
The `search` method performs a general search on the DataTable based on an array of search conditions and a search type,
...
`rollBack()`  will always return `false`. No exceptions will be thrown.

### InMemoryDataTable

A `DataTable` implementation using simple PHP arrays, no storage, no transactions. This makes
it possible to perform some tests on data tables without having to set up a database.
...
```
$dt = new MySqlDataTable($pdoOrProvider, $mySqlTableName, $useAutoInc, $idColumnName);
```

A `PDO` object or a `PdoProvider` implementation can be used to give `MySqlDataTable` a database connection to
work with. `PdoProvider` is an interface with a single method: `getPdo()`. `MySqlDataTable` will use that method
whenever a connection is needed and throw an exception if the provider does not provide a connection.

`MySqlDataTable` assumes that there is a table setup with at least an int or bigint `id` column with the given name
(`$idColumnName`, which defaults to 'id').

If `$useAutoInc` is false,`MySqlDataTable` itself takes care of generating incremental IDs by default and
`setGeneratorId` can be used to use a different id generation strategy.

If `$useAutoInc` is true, `MySqlDataTable` assumes that the `id` column has the `AUTO_INCREMENT` attribute in the
database and rows will be created letting the database handle new id generation. Calls to `setGeneratorId` will throw an
error.
...
as calls to `createRow` and `updateRow` agree with column names and types, everything
...
### UnitemporalDataTable and MySqlUnitemporalDataTable

An UnitemporalDataTable is an interface that extends DataTable. It is a DataTable with time-tagged rows.
Every row not only has a unique id, but also a `valid_from` and a `valid_until` time. When using the normal `DataTable`
methods `UnitemporalDataTable` behaves exactly the same as a DataTable, but it does not delete any rows,
it just makes them invalid.

The package provides MySql implementation: `MySqlUnitemporalDataTable`, which is used in the examples below. Any
`UnitemporalDataTable` implementation should behave the same way.

There is a set of time methods to create, read, update and delete previous versions of the data. For example:

``` 
$dt = new MySqlUnitemporalDataTable($pdoOrPdoProvider, $mySqlTableName);

$oldRow = $dt->getRowWithTime($rowId, $timeString);
```

`$timeString ` is a string formatted as a valid MySQL datetime with microseconds, e.g. `'2018-01-01  12:00:00.123123'`.
Use the static methods in the `TimeString` class (provided by the `thomas-institut/timestring` dependency) to generate
such strings from MySQL date and datetime strings, and from UNIX timestamps with or without microseconds.

The underlying database table must have two datetime fields: `valid_from` and `valid_until`

The user is responsible for setting the database connection with the timezone that is going to be used in all queries
